...
import static com.cxwl.hurry.doorlock.config.Constant.MSG_RESTART_VIDEO1;
...
import static com.cxwl.hurry.doorlock.config.Constant.START_FACE_CHECK;
...
//        44:2c:05:e6:9c:c5  13区
//        cc:4b:73:0a:5f:56  华铁
//^(?!.*(WifiStateMachine|FrameHelper|DB|OMX_CAM|CameraHal|进程|okhttp)).*$
    }
...
//                        }
    ...
    ArrayList<GuangGaoBean> obj1;

    public void onAdvertiseRefreshPic(ArrayList<GuangGaoBean> obj) {
        startTime = System.currentTimeMillis() + "";
        obj1 = obj;
        if (obj1.size() > 0) {
            Iterator<GuangGaoBean> it = obj1.iterator();
            while (it.hasNext()) {
                if ((Long.parseLong(StringUtils.transferDateToLong(it.next().getShixiao_shijian())) < Long.parseLong
                        (startTime))) {
                    it.remove();
                    Log.i(TAG, "广告图片已经失效停止这张图片的播放" + obj1.size());
                }
            }
//            for (int i = obj1.size() - 1; i > 0; i--) {
//                if (Long.parseLong(StringUtils.transferDateToLong(obj1.get(i).getShixiao_shijian())) < Long.parseLong
//                        (startTime)) {
//                    //表示这张图片已经失效不在进行播放
//                    boolean remove = obj1.remove(obj1.get(i));
//                    Log.i(TAG, "广告图片已经失效停止这张图片的播放"+remove+obj1.size());
//
//                }
//            }
        }
        ...
        Log.d(TAG, "banner加载图片 size" + obj1.size() + "banner.isActivated()" + banner.isActivated());
        if (obj1.size() == 0) {
        ...
        banner.setOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int i) {
                ...
                if (Long.parseLong(StringUtils.transferDateToLong(obj1.get(i).getShixiao_shijian())) < Long.parseLong
                        (endTime)) {
                    Log.i(TAG, "广告图片已经失效停止这张图片的播放");

                    //表示失效了

                    if (obj1.size() == 2) {
                        //表示只有两张图片 而且可能一样
                        if (obj1.get(0).getId() == obj1.get(1).getId()) {
                            //表示同一张图片
                            obj1.clear();
                            onAdvertiseRefreshPic(obj1);

                        } else {
                            //表示不同的图片
                            obj1.remove(obj1.get(i));
                            Log.i("图片已经obj1", obj1.toString());
                            //  obj1.add(obj1.get(0));
                            //    banner.setDelayTime(10000);
                            onAdvertiseRefreshPic(obj1);
                        }
            ...
            if (DeviceConfig.DEVICE_TYPE.equals("B") && netWorkFlag == 1) {//大门,人脸验证走网络
                if (DeviceConfig.PRINTSCREEN_STATE == 0) {
                    if (mImageNV21 != null && identification) {//摄像头检测到人脸信息且处于人脸识别状态
                        ...
                        // TODO: 2018/7/25 1.转换人脸信息(result)为.bin文件存在本地tempface文件夹中
                        // TODO: 2018/7/25 2.将.bin文件提交给服务器判断(同步或异步提交)
                        commitData();
                        // TODO: 2018/7/25 同步start
                        if (true) {//匹配，开门
                        ...
                        mAFT_FSDKFace = null;
                        mImageNV21 = null;
                        // TODO: 2018/7/25 同步end

                        // TODO: 2018/7/25 异步start
                        uploadBin();
                        // TODO: 2018/7/25 异步end
                    }
                }
            } else {//单元门,人脸验证走本地
